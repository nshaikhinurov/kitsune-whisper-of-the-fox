...
// A tile factory closes over the game's RNG and a monotonic id counter. The id
// counter is deliberately OUTSIDE the RNG stream so tile identity (used only for
// client animations) never perturbs the element/heart draws — that keeps the
// stream byte-identical between client and server. Element is drawn before
// hasHeart, matching the original randomTile() property order.
export type TileFactory = () => TileState;

export function createTileFactory(rng: Rng): TileFactory {
  let nextId = 0;
  return () => {
    ...
    return {
      ...
      hasHeart: rng() < HEART_SPAWN_CHANCE,
    };
  };
}
...
export function createBoard(
  ...
  do {
    board = Array.from({ length: rows }, () =>
      Array.from({ length: cols }, () => makeTile()),
    );
...
export function refillBoard(
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (next[r][c] === null) next[r][c] = makeTile();
    }
  }
